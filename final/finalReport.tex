...
\textbf{BusyBox:} BusyBox supports creation of a shared library out of the box. The following steps are needed to perform to generate the shared library once you download the Busybox source code:
\begin{itemize}
  \item {\tt cd busybox-1.22.1} - Go to the Busybox directory that you have downloaded
  \item {\tt make menuconfig } - Once you entered the command, you will see a graphical configuration menu, where you will have to disable static binary option and enable shared library option
  \item {\tt make } - Compile the source codes to generate the object files and finally the shared library for busy box
\end{itemize}  
This shared library was used later with the tester program, to directly call the echo implementation of busybox.

\textbf{Coreutils:} Coreutils does not support creation of a library out of the box unlike Busybox; we had to manually compile echo program of Coreutils. While doing so we observed that echo.c file, which contains Coreutils implementation, was dependent on the other static libraries such as {\tt libcoreutils.a} and {\tt libver.a}. There are two criteria to compile a source file into a shared library. Namely, the dependent libraries must be compiled with  {\tt -fPIC} flag with GCC and if they are not compiled with  {\tt -fPIC} flag then the libraries should be 32 bit architecture type. Since {\tt libcoreutils.a} and {\tt libver.a} are not compiled with fPIC and they are compiled with 64 bit, we could not generate a shared library of the echo program. 

The last resort was to generate a static library that can be compiled with the tester program. The following commands were used to build the static library of coreutils echo program once coreutils source files have been downloaded locally: 
\begin{itemize}
  \item {\tt cd coreutils-8.23} - Go to the Coreutils directory
  \item {\tt ./configure} - Configure the flags depending on the system and OS environment
  \item {\tt make all} - compile source files into the respective binaries. In this step, we generate the dependent libraries that are needed for the echo program
  \item {\tt cd src}
  \item {\tt gcc -std=gnu99 -I../lib -O2 -g -c -o echo.o echo.c}
  \item {\tt ld -r -o tmpecho.o echo.o ../lib/libcoreutils.a libver.a} 
  \item {\tt ar -cvq libecho.a tmpecho.o} - libecho.a is the static library of the echo program, which we will use against the tester program. 
\end{itemize}

\textbf{Other challenges:} We performed aforementioned tasks, shared library for Busybox and static library for Coreutils' echo, on a Ubuntu 64 bit system. We successfully built the respective libraries and compiled against the tester program, {\tt test.c}, using the following command: \\\\
	{\tt export LD\_LIBRARY\_PATH=/home/s2e/:\$LD\_LIBRARY\_PATH
	gcc -o test test.c libecho.a -L. -lbusybox }
	 \\\\
Next, we tried to compiled the tester program on a QEMU emulator that S2E executes on, however, it failed. The major reason behind this was the fact that the libraries were compiled on the 64bit system and QEMU emulator was 32 bit. We installed 32bit Debian OS, which was also used by S2E's QEMU emulator, and generated those libraries. Then we were successful at getting the tester program compiled against the libraries, and we were able to execute each of the {\tt echo} implementations from the tester program.
